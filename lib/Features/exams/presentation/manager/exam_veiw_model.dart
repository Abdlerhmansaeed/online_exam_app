...
class ExamViewModel extends Cubit<ExamStates> {
  ...
  Future<void> getExamsOnSubject(String subjectId) async {
    return handleCubitStates<GetExamsOnSubject>(
      ...
      onSuccess: (data) async {
        if (data.exams != null) {
          await _examLocalDataSource?.saveExams(data.exams!); // Save exams in Hive
        }
        emit(state.copyWith(
          examOnSubjectStates: SuccessState<GetExamsOnSubject>(data),
        ));
      },
      onError: (error) => emit(state.copyWith(examOnSubjectStates: ErrorState(error))),
      onLoading: () => emit(state.copyWith(examOnSubjectStates: LoadingState())),
    );
  }
      ...
      onSuccess: (data) {
        ...
        for (var q in questions) {
          answers.addAll(q.answers
                  ?.where((examAnswers) => examAnswers.answer != null)
                  .map((examAnswers) =>
                      ExamAnswers(key: q.Id, answer: examAnswers.answer)) ??
              []);
        }
  ...
  void goToPreviousQuestion() {
    ...
    if (previousIndex >= 0) {
      ...
      emit(
        state.copyWith(currentQuestionIndex: previousIndex),
      );
    }
  }
  ...
  void selectedUserAnswer(
    ...
    final existingIndex = newUserAnswers
        .indexWhere((answers) => answers.questionId == questionId);
  ...
  Future<void> checkUserAnswers() async {
    return handleCubitStates<CheckQuestionsResponse>(
      request: () async {
        ...
        final userAnswerList = userAnswers
            .map<UserAnswers>((answer) => UserAnswers(
    ...
    _timer = Timer.periodic(
      oneSec,
        ...
        if (examDurationInSeconds == 0) {
          ...
          if (examDurationInSeconds <= 900 && !(state.isDangerTime ?? false)) {
            emit(state.copyWith(
              examDuration: examDurationInSeconds,
              isDangerTime: true,
            ));
          } else {
